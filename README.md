...
* Left pane is the shell's own navigation control (`CLSID_NamespaceTreeControl`)
  — the same one Explorer uses, so drives, folders, icons and expansion behave
  identically. Selecting a folder filters the list to sessions at or below it.
...
pointer-free messages (`WM_LBUTTONDOWN`, `WM_CHAR`, `WM_COMMAND`, …) are used,
and clicks are posted rather than sent, because a tree view's button-down
handler runs a nested loop waiting for the button-up.
